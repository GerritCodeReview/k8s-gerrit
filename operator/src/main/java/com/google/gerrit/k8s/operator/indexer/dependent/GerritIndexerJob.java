...
public class GerritIndexerJob
  ...
  private List<VolumeMount> buildCommonVolumeMounts(GerritIndexer gerritIndexer) {
    ...
    volumeMounts.add(
        new VolumeMountBuilder()
            .withName("gerrit-repositories")
            .withMountPath("/var/mnt/git")
            .withSubPath(gerritIndexer.getSpec().getStorage().getRepositories().getSubPath())
            .withReadOnly(true)
            .build());
  ...
  private List<Volume> buildVolumes(GerritIndexer gerritIndexer, GerritCluster gerritCluster) {
    ...
    volumes.add(
        new VolumeBuilder()
            ...
            .withClaimName(siteClaimName(gerritIndexer))
    ...
    if (isSeparateOutputVolume(gerritIndexer)) {
      volumes.add(
          new VolumeBuilder()
              ...
              .withNewPersistentVolumeClaim()
              .withClaimName(repositoriesClaimName(gerritIndexer))
              .endPersistentVolumeClaim()
              .build());
      volumes.add(
          new VolumeBuilder()
              .withName("index-output")
              .withNewPersistentVolumeClaim()
              .withClaimName(
  ...
  private String repositoriesClaimName(GerritIndexer gerritIndexer) {
    String name = gerritIndexer.getSpec().getStorage().getRepositories().getPersistentVolumeClaim();

    if (name == null || name.isBlank()) {
      name = GerritRepositoriesSnapshotPVC.getName(gerritIndexer);
    }
    return name;
  }

  private String siteClaimName(GerritIndexer gerritIndexer) {
    String name = gerritIndexer.getSpec().getStorage().getSite().getPersistentVolumeClaim();

    if (name == null || name.isBlank()) {
      name = GerritSiteSnapshotPVC.getName(gerritIndexer);
    }
    return name;
  }
