...
public class GerritSiteSnapshotPVC
    extends CRUDReconcileAddKubernetesDependentResource<PersistentVolumeClaim, GerritIndexer> {

  public GerritSiteSnapshotPVC(Class<PersistentVolumeClaim> resourceType) {
    super(resourceType);
  }
  ...
  protected PersistentVolumeClaim desired(
    ...
    return new PersistentVolumeClaimBuilder()
        ...
        .withNewResources()
        .withRequests(
            Map.of("storage", getPrimaryGerrit(gerritCluster).getSpec().getSite().getSize()))
        .endResources()
        .withNewDataSource()
        .withApiGroup("snapshot.storage.k8s.io")
        .withKind("VolumeSnapshot")
        .withName(GerritSiteVolumeSnapshot.getName(gerritIndexer))
        .endDataSource()
        .endSpec()
        .build();
  }
  ...
  public static String getName(String gerritIndexerName) {
    return gerritIndexerName + "-site-snapshot-pvc";
  }
