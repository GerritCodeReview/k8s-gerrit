...
public class FluentBitConfigMap extends CRUDKubernetesDependentResource<ConfigMap, Gerrit> {
  ...
  protected ConfigMap desired(Gerrit gerrit, Context<Gerrit> context) {
    String config = gerrit.getSpec().getFluentBitSidecar().getConfig();
    String configName = gerrit.getSpec().getFluentBitSidecar().getConfigName();
    ...
    return new ConfigMapBuilder()
        ...
        .endMetadata()
        .withData(Map.of(configName, config))
        .build();
  }
}
