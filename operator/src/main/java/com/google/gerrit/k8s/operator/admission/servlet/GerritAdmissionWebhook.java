...
public class GerritAdmissionWebhook extends ValidatingAdmissionWebhookServlet {
  ...
  private boolean noRefDbConfiguredForHA(Gerrit gerrit) {
    return gerrit.getSpec().isHighlyAvailablePrimary() && noRefdbConfigured(gerrit);
  }

  private boolean noRefDbConfiguredForMultisite(Gerrit gerrit) {
    return OperatorContext.isMultisite() && noRefdbConfigured(gerrit);
  }

  private boolean noRefdbConfigured(Gerrit gerrit) {
