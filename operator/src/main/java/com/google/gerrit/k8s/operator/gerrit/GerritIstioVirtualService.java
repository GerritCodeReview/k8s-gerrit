...
public class GerritIstioVirtualService
  ...
  protected VirtualService desired(Gerrit gerrit, Context<Gerrit> context) {
    ...
    return new VirtualServiceBuilder()
        ...
        .withTcp(getTCPRoutes(gerritCluster, gerrit))
        .endSpec()
        .build();
  }
  ...
  private List<TCPRoute> getTCPRoutes(GerritCluster gerritCluster, Gerrit gerrit) {
    List<TCPRoute> routes = new ArrayList<>();
    if (gerritCluster.getSpec().getIngress().isEnableSsh()) {
      routes.add(
          new TCPRouteBuilder()
