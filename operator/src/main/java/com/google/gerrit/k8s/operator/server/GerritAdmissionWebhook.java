...
public class GerritAdmissionWebhook extends ValidatingAdmissionWebhookServlet {
  ...
  private boolean missingSpannerConfig(Gerrit gerrit) {
    return gerrit.getSpec().getRefdb().getDatabase().equals(GlobalRefDbConfig.RefDatabase.SPANNER)
        && gerrit.getSpec().getRefdb().getSpanner() == null;
  }
