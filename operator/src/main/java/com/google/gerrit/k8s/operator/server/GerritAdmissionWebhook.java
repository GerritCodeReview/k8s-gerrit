...
public class GerritAdmissionWebhook extends ValidatingAdmissionWebhookServlet {
  ...
  Status validate(HasMetadata resource) {
    ...
    if (!onlyOnePrimaryGerritInCluster(gerrit)) {
      return new StatusBuilder()
  ...
  private boolean clusterNameSet(Gerrit gerrit) {
    String clusterName = gerrit.getSpec().getCluster();
    return clusterName != null && !clusterName.isBlank();
  }

  private boolean onlyOnePrimaryGerritInCluster(Gerrit gerrit) {
    ...
    return client
        ...
        .getItems()
        .stream()
        .noneMatch(g -> clusterName.equals(g.getSpec().getCluster()));
  }
