...
public class GerritHeadlessService extends GerritAbstractService {
  ...
  private void migrateFromHeadService(Gerrit gerrit, KubernetesClient client) {
    ...
    Optional<Service> optionalExistingService = Optional.ofNullable(serviceResource.get());
    if (optionalExistingService.isPresent()) {
      Service existingService = optionalExistingService.get();
      if (!existingService.getSpec().getClusterIP().toLowerCase().equals("none")) {
        logger.atInfo().log(
            "Deleting old headful service (%s/%s) to replace it with headless service.",
            existingService.getMetadata().getNamespace(), existingService.getMetadata().getName());
        serviceResource.withTimeoutInMillis(10000).delete();
      }
    }
  }
