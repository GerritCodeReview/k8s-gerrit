...
@pytest.fixture(scope="session")
def docker_build(request, docker_client, tag_of_cached_container, docker_registry,
                 docker_org, docker_tag):
...
@pytest.fixture(scope="session")
def docker_push(request, docker_client, docker_registry, docker_login,
                docker_org, docker_tag):
...
@pytest.fixture(scope="session")
def apache_git_http_backend_image(request, container_images, docker_build,
                                  docker_push, base_image):
...
@pytest.fixture(scope="session")
def gerrit_master_image(request, container_images, docker_build, docker_push,
...
@pytest.fixture(scope="session")
def gerrit_slave_image(request, container_images, docker_build, docker_push,
                       base_image, gerrit_base_image):
...
@pytest.fixture(scope="session")
def gerrit_init_image(request, container_images, docker_build, docker_push,
                      base_image, gerrit_base_image):
...
@pytest.fixture(scope="session")
def mysql_replication_init_image(request, container_images, docker_build,
                                 docker_push):
  mysql_replication_init_image = docker_build(
    container_images["mysql-replication-init"], "mysql-replication-init")
  if request.config.getoption("--push"):
    docker_push("mysql-replication-init")
  return mysql_replication_init_image
