...
def init_container(request, docker_client, docker_network,
    mysql_replication_init_image, mock_sql_script, mysql_container):
...
class TestMysqlInitScript(object):
  def test_mysql_replication_init_waiting_for_dump(self, init_container):
    time.sleep(5)
  ...
  def test_mysql_replication_init_accepts_dump(self, init_container, mock_dump):
  ...
  def test_mysql_replication_init_runs_slave_init_script(self, init_container):
    connection = connect_to_mysql()
