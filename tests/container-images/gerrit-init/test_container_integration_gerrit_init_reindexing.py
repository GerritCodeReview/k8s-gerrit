...
class TestGerritReindex:
    ...
    def test_gerrit_init_skips_reindexing_on_fresh_site(
        ...
        exit_code, _ = container_run_endless.exec_run(
            "/var/tools/gerrit-initializer -s /var/gerrit -c /var/config/default.config.yaml init"
        )
        assert exit_code == 0
        ...
        exit_code, _ = container_run_endless.exec_run(
            "/var/tools/gerrit-initializer -s /var/gerrit -c /var/config/default.config.yaml reindex"
        )
        assert exit_code == 0
