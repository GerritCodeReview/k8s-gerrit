...
def config_file_default(tmp_path_factory):
  ...
  with open(os.path.join(tmp_config_dir, "gerrit.config"), "w") as gerrit_config:
    gerrit_config.write("""
    [gerrit]
      basePath = git
  ...
  with open(os.path.join(tmp_config_dir, "secure.config"), "w") as secure_config:
    secure_config.write("""
    [database]
      username = root
      password = secret
...
class TestGerritInitMySQLValidateDB(object):
  def test_db_server_is_found(self, container_run_mysql_validate_db,
                              mysql_container):
  ...
  def test_gerrit_init_waits_for_configured_db(
      self, container_run_mysql_validate_db, mysql_container):

    for _ in range(10):
      log = container_run_mysql_validate_db.logs(tail=1).decode("utf-8")
      assert re.search(r"Still waiting", log)

  def test_gerrit_init_detects_database_available(
      self, container_run_mysql_validate_db, mysql_container):
  ...
  def test_gerrit_init_waits_for_schema(
      self, container_run_mysql_validate_db, mysql_container):
  ...
  def test_gerrit_init_detects_schema_available(
      self, container_run_mysql_validate_db, mysql_container):
    connection = mysql_container.connect()
