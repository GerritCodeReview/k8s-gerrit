...
def config_default(tmp_path_factory):
    ...
    [httpd]
      listenUrl = http://*:8080
    """.format(
        host=MYSQL_CONFIG["MYSQL_HOST"], port=MYSQL_CONFIG["MYSQL_PORT"]
    )
...
def gerrit_slave_container(
    ...
    configs = {"gerrit.config": config_default[0], "secure.config": config_default[1]}
...
def container_run_factory(
    request, docker_client, docker_network, gerrit_init_image, tmp_path_factory
):
    def container_run(config_path, entrypoint=None, command=None):
        ...
        container_run = docker_client.containers.run(
            ...
            command=command,
            volumes={
                empty_site: {"bind": "/var/gerrit/", "mode": "rw"},
                config_path: {"bind": "/var/gerrit/etc", "mode": "rw"},
            },
...
def container_run_gerrit_init_manual(container_run_factory, config_file_default):
    return container_run_factory(
        config_file_default, entrypoint="tail", command=["-f", "/dev/null"]
    )
...
class TestGerritInitMySQLForSlave:
    def test_site_initialized(self, container_run_gerrit_init_manual, mysql_container):
        ...
        container_run.exec_run("/var/tools/gerrit_init.py -s /var/gerrit")
