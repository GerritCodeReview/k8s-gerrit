...
class EFSProvisioner(AbstractStorageProvisioner):
  ...
  def deploy(self):
    ...
    res = self.helm.install("stable/efs-provisioner", self.name,
                            set_values=chart_opts, fail_on_err=False)
...
class TestCluster:
  ...
  def _load_kube_config(self):
    ...
    _, context = \
      config.list_kube_config_contexts(config_file=self.kube_config)
  ...
  def _create_and_deploy_helm_crb(self):
    ...
    crb = client.V1ClusterRoleBinding(
      metadata=crb_meta,
      role_ref=crb_role_ref,
      subjects=crb_subjects
    )
  ...
  def create_image_pull_secret(self, namespace="default"):
    ...
    auth_string = str.encode("%s:%s" % (self.registry["user"], self.registry["pwd"]))
    secret_data = {
      "auths": {
        self.registry["url"]: {
    ...
    secret_body = client.V1Secret(
      ...
      data={
        ".dockerconfigjson": base64.b64encode(secret_data).decode("utf-8")
      }
    )
    core_v1 = client.CoreV1Api()
    try:
  ...
  def remove_helm(self):
    ...
    for replica_set in replica_sets.items:
      print(replica_set.metadata.name)
      if re.match(r"tiller-deploy-.*", replica_set.metadata.name):
        print("matched")
        apps_v1.delete_namespaced_replica_set(
          replica_set.metadata.name, "kube-system", body=client.V1DeleteOptions())
        break
    core_v1 = client.CoreV1Api()
    core_v1.delete_namespaced_service_account(
      HELM_SERVICE_ACCOUNT_NAME, HELM_SERVICE_ACCOUNT_NAMESPACE,
...
@pytest.fixture(scope="session")
def namespace_factory(test_cluster):
  ...
  def create_namespace(name):
    ...
    test_cluster.create_image_pull_secret(name)
