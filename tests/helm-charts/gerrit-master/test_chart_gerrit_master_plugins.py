...
def plugin_list():
  ...
  for plugin in PLUGINS:
    url = ("https://gerrit-ci.gerritforge.com/view/Plugins-stable-{gerrit_version}/"
           "job/plugin-{plugin}-bazel-stable-{gerrit_version}/lastSuccessfulBuild/"
           "artifact/bazel-bin/plugins/{plugin}/{plugin}.jar").format(
...
@pytest.fixture(
  ...
  ids=[
    "single-packaged-plugin",
    "multiple-packaged-plugins"
  ])
...
def gerrit_master_deployment_with_other_plugins(
  ...
  for counter, plugin in enumerate(selected_plugins):
    chart_opts["gerritMaster.plugins.downloaded[%d].name" % counter] = plugin["name"]
    chart_opts["gerritMaster.plugins.downloaded[%d].url" % counter] = plugin["url"]
    chart_opts["gerritMaster.plugins.downloaded[%d].sha1" % counter] = plugin["sha1"]
  chart = gerrit_master_deployment_with_plugins_factory(chart_opts)
...
def gerrit_master_deployment_with_other_plugin_wrong_sha(
  ...
  chart = gerrit_master_h2_deployment_factory(chart_opts, wait=False)
...
class TestGerritMasterChartPackagedPluginInstall:
  ...
  def test_install_packaged_plugins(
    ...
    self._assert_installed_plugins(
      gerrit_master_deployment_with_packaged_plugins["installed_plugins"], response)
  ...
  def test_install_packaged_plugins_are_removed_with_update(
    ...
    update_chart(
      test_cluster.helm, chart, {"gerritMaster.plugins.packaged": plugins_opts_string}
    )
  ...
  def _remove_plugin_from_install_list(self, installed_plugins):
    ...
    if installed_plugins:
      for counter, plugin in enumerate(installed_plugins):
        plugin_install_list["gerritMaster.plugins.downloaded[%d].name" % counter] = \
          plugin["name"]
        plugin_install_list["gerritMaster.plugins.downloaded[%d].url" % counter] = \
          plugin["url"]
        plugin_install_list["gerritMaster.plugins.downloaded[%d].sha1" % counter] = \
          plugin["sha1"]
    else:
  ...
  def test_install_other_plugins_are_removed_with_update(
    ...
    while True:
      try:
        response = get_gerrit_plugin_list("http://master.%s" % (
          request.config.getoption("--ingress-url")))
        if response is not None and chart["removed_plugin"]["name"] not in response:
          break
      except requests.exceptions.ConnectionError:
        time.sleep(1)
...
def test_install_other_plugins_fails_wrong_sha(
  ...
  while not current_status:
    ...
    for init_container_status in pod.status.init_container_statuses:
      if init_container_status.name == "gerrit-init" and \
          init_container_status.last_state.terminated:
