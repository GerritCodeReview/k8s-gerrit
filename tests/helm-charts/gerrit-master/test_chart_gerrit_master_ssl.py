...
def ssl_certificate(request, cert_dir):
  ...
  open(os.path.join(cert_dir, "server.crt"), "wb").write(
    keypair.get_cert())
  open(os.path.join(cert_dir, "server.key"), "wb").write(
    keypair.get_key())
  return keypair

@pytest.fixture(scope="class")
def gerrit_master_deployment_with_ssl(
    request, docker_tag, test_cluster, ssl_certificate, gerrit_master_h2_deployment_factory):
  chart_opts = {
...
class TestGerritMasterChartSetup(object):
  ...
  def test_create_project_rest(self, request, cert_dir,
                               gerrit_master_deployment_with_ssl):
  ...
  def test_cloning_project(self, request, tmp_path_factory, test_cluster,
    ...
    repo_url = "https://master.%s/test.git" % (
      request.config.getoption("--ingress-url"))
    repo = git.Repo.clone_from(repo_url, clone_dest, env={"GIT_SSL_NO_VERIFY": "true"})
