...
class PushProjectAction(abstract.AbstractAction):

  def __init__(self, project_name, probability=0.05):
  ...
  def execute(self):
    ...
    if os.path.exists(local_repo_path) and self._is_executed():
      ...
      for _ in range(num_commits):
        self._create_commit()
      self.log.info(
        "Pushing %d changes to project %s", num_commits, self.project_name)
      self.repo.remotes.origin.push(refspec="HEAD:master")
      self.was_executed = True

  def _create_commit(self):
    change_type_choice = random.choice(self.CHANGE_TYPES)
    change_type_choice()
    self.repo.index.commit(self._create_random_string(16))
