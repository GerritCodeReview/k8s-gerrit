...
class GitGarbageCollectionProvider:
    ...
    def get(site_path, create_bitmap=True, pack_refs=True, preserve_packs=False):
        ...
        if preserve_packs:
            ...
            gc_config = f"{gc_config} -c gc.packRefs=false"

        if pack_refs:
            after_steps.append(PackRefsAfterStep())

        return GitGarbageCollection(
            site_path, init_steps, after_steps, gc_config.strip()
        )


class GitGarbageCollection:
    def __init__(self, site_path, init_steps, after_steps, gc_config=""):
