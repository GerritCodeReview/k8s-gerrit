...
class PreservePacksInitStep(GCStep):
    ...
    def _prune_preserved(self, repo):
        ...
        if os.path.exists(full_preserved_pack_path):
            ...
            for file in os.listdir(full_preserved_pack_path):
                if os.path.isfile(file) and file.endswith(".old_pack"):
                    count += 1
    ...
    def _preserve_packs(self, repo):
        ...
        for file in os.listdir(full_pack_path):
            ...
            if os.path.isfile(full_file_path) and filename.startswith("pack-") and ext in [".pack", ".idx"]:
                LOG.debug("Preserving pack %s" % file)
                os.link(os.path.join(full_pack_path, file), os.path.join(full_preserved_pack_path, self._get_preserved_packfile_name(file)))
                if ext == ".pack":
                    count += 1
...
class PackRefsAfterStep(GCStep):
    def run(self, repo):
        loose_ref_count = len(
            [
                file
                for file in os.listdir(os.path.join(repo.git_dir, "refs"))
