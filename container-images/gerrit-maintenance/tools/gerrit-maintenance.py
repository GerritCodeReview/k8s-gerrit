...
def cli_projects(ctx, skip, project):
    """Commands for maintaining git repositories"""
    ctx.obj["SKIP_PROJECTS"] = skip
    ctx.obj["PROJECTS"] = project
...
def gc(ctx, bitmap, pack_refs, preserve_packs):
    ...
    ).gc(ctx.obj["PROJECTS"], ctx.obj["SKIP_PROJECTS"])
