...
class GerritInit:
    def __init__(self, site, config):
        ...
        self.gerrit_config = git.GitConfigParser(os.path.join(self.site, "etc/gerrit.config"))
    ...
    def _symlink_configuration(self):
        ...
        if os.path.exists(f"{MNT_PATH}/etc/config"):
            for file_or_dir in os.listdir(f"{MNT_PATH}/etc/config"):
                if os.path.isfile(file_or_dir):
                    self._ensure_symlink(
                        os.path.join(f"{MNT_PATH}/etc/config", file_or_dir),
                        os.path.join(etc_dir, file_or_dir),
                    )
        if os.path.exists(f"{MNT_PATH}/etc/secrets"):
            for file_or_dir in os.listdir(f"{MNT_PATH}/etc/secret"):
                if os.path.isfile(file_or_dir):
                    self._ensure_symlink(
                        os.path.join(f"{MNT_PATH}/etc/secret", file_or_dir),
                        os.path.join(etc_dir, file_or_dir),
                    )
    ...
    def execute(self):
        ...
        pid_file = f"{self.site}/logs/gerrit.pid"
