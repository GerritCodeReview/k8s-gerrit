...
class PullReplicationConfigurator:
    ...
    def _configure_remotes(self, template_path):
        ...
        options = config.list()
        gerrit_ops = git.GitConfigParser.get_section_as_string(options, 'gerrit', True, True)
        replication_ops = git.GitConfigParser.get_section_as_string(options, 'replication', True, True)
