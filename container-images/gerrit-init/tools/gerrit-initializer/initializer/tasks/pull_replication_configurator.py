...
class PullReplicationConfigurator:
    def __init__(self, site):
        ...
        self.remote_host_postfix = self._get_remote_host_postfix()
        self.replicas = self._get_replicas_num()

    @staticmethod
    def has_pull_replication():
        return (os.path.exists(os.path.join(MNT_PATH, 'etc/config/replication.config')) and
                (os.path.exists('/var/gerrit/lib/pull-replication.jar') or
                 os.path.exists('/var/gerrit/plugins/pull-replication.jar')))

    def _get_pod_identifier(self):
        return int(os.environ.get('POD_INDEX'))
    ...
    def _configure_remotes(self, template_path):
        ...
        for idx, x in enumerate(remote_pod_ids):
            content = content.replace(f'{REMOTE_ID_PLACEHOLDER}-{idx + 1}', f'gerrit-{x}')
            content = content.replace(f'{REMOTE_HOST_PLACEHOLDER}-{idx + 1}', f'gerrit-{x}.{self.remote_host_postfix}')
    ...
    def configure_pull_replication(self):
        LOG.info('Setting pull replication configuration for pod-idx: {}'.format(self.pod_id))

        replication_config_configmap = os.path.join(MNT_PATH, 'etc/config/replication.config')
