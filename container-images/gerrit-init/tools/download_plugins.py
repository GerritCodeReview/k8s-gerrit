...
class CachedPluginInstaller(AbstractPluginInstaller):
  ...
  def _cleanup_cache(plugin_cache_dir):
    ...
    while len(cached_files) > MAX_CACHED_VERSIONS:
      ...
      LOG.info("Too many cahced files in %s. Removing file %s",
               plugin_cache_dir, oldest_file)
  ...
  def _install_plugin(self, plugin):
    ...
    if os.path.exists(install_path) and self._compare_plugin_sha(plugin, install_path):
      return
...
def get_installer(site, config):
  plugin_installer = CachedPluginInstaller if config.plugin_cache_enabled else PluginInstaller
