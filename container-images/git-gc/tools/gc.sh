...
set_gc_command() {
  if [ $PRESERVE_PACKS_OPT -eq 0 ] ; then
    ...
    export PATH="$PATH;/var/tools/git-gc-preserve"
...
gc_project()
{
  ...
  OUT=$(find "$PROJECT_DIR/objects" -name 'incoming_*.pack' -type f -mtime +14 -ls -delete) && \
