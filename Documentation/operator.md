...
```sh
VERSION=1.20.3 # Should be the same version as set in istio/gerrit.profile.yaml
OS=osx
ARCHITECTURE=arm64
curl \
  -L https://github.com/istio/istio/releases/download/$VERSION/istioctl-$VERSION-$OS-$ARCHITECTURE.tar.gz \
  -o /tmp/istioctl.tar.gz
sudo tar -xf /tmp/istioctl.tar.gz -C /usr/local/bin/
```
