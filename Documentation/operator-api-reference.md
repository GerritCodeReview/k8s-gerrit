...
| `ambassadorId` | `List<String>` | Only used when the INGRESS environment variable is set to "ambassador" in the operator. The operator uses the ids specified in `ambassadorId` to set the [ambassador_id](https://www.getambassador.io/docs/edge-stack/1.14/topics/running/running#ambassador_id) spec field in the Ambassador CustomResources it creates (`Mapping`, `TLSContext`). (optional) |
