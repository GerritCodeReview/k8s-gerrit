...
[Fluent Bit](https://docs.fluentbit.io/manual/administration/configuring-fluent-bit/classic-mode/configuration-file)
is installed as a sidecar container to each Gerrit pod, which allows application
logs to be collected. If no host or port is set, logs will still be available
through stdout in each sidecar.

| Field | Type | Description |
|---|---|---|
| `image` | `String` | Fluent Bit image from docker (default: `fluent/fluent-bit:latest`) |
| `outputHost` | `String` | Host address to send Gerrit logs to via forward protocol |
| `outputPort` | `String` | Port to send Gerrit logs to |
